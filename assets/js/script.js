$(function () {
    ...
    function renderGitHubProjects(repos) {
        ...
        const html = visible
            .map(function (repo, index) {
                ...
                if (stars > 0) meta.push(`<span>&#9733;&nbsp;${stars}</span>`);
                if (forks > 0) meta.push(`<span>Forks&nbsp;${forks}</span>`);

                const demoHtml = demoImage
                    ? `<div class="project-demo-img"><img src="${escapeHtml(demoImage)}" alt="${name} preview" loading="lazy" /></div>`
                    : "";

                const liveLink = safeLive
                    ? `<a class="project-link project-link-live" href="${safeLive}" target="_blank" rel="noopener noreferrer" aria-label="View live demo of ${name} (opens in new tab)">Live Demo &rarr;</a>`
                    : "";
                const githubLink = `<a class="project-link project-link-github" href="${safeGithub}" target="_blank" rel="noopener noreferrer" aria-label="View ${name} source code on GitHub (opens in new tab)">GitHub &rarr;</a>`;

                return `<article class="project-card reveal" data-delay="${delay}">
                ${demoHtml}
                <div class="project-content">
                    <span class="project-number">${number}</span>
                    <h3>${name}</h3>
                    <p>${description}</p>
                    <div class="project-tech">${meta.join("")}</div>
    ...
    function startCertTimer() {
        ...
        certTimer = window.setInterval(function () {
            if (!certPaused)
                renderCert((certIndex + 1) % CERTIFICATE_ITEMS.length);
        }, CERT_AUTOPLAY_MS);
    }
    ...
    function renderCert(index) {
        ...
        const $img = $("#cert-img");
        $img.attr("src", cert.image || "").attr(
            "alt",
            cert.title ? "Certificate: " + cert.title : "Certificate",
        );
        ...
        if (cert.download && cert.download.trim()) {
            $download
                ...
                .attr(
                    "aria-label",
                    "Download certificate: " + (cert.title || ""),
                )
                .show();
        } else {
            $download.hide();
    ...
    function updateActiveNav() {
        ...
        $sections.each(function () {
            ...
            if (
                marker >= offset.top &&
                marker < offset.top + $s.outerHeight()
            ) {
                currentId = $s.attr("id");
            }
        });
        ...
        if (!currentId) {
            ...
            if ($home.length) {
                ...
                if (ho && scrollTop < ho.top + $home.outerHeight())
                    currentId = "home";
            }
        }
